    ...
    pool.getConnection(function (errz, connz) {
      ...
      connz.query('select table_name as tn from information_schema.tables where table_schema = "' + pool.database + '"', [], function (erry, rsy) {
        ...
        for (var i in tblarrs) {
          ...
          // console.log('in yorm.bufferinit. ssqldef: ' + ssqldef)
        ...
        Promise.all(pros).then(function () {
          // console.log('=============Resolveing successfully: ' + JSON.stringify(result))
          module.exports.tbldefs = result
          resolve();
        }).catch(function (errs) {
          console.log("Get erros in bufferinit(). errs is : " + errs);
          reject(errs)
        })
      })
    })
...
exports.bufferinit().then(function () {
  ...
  // console.log('yorm exports defined..')
})
...
var query = function(ssql) {
  return new Promise(function (resolve, reject) {
    pool.getConnection(function(err, conn) {
      conn.query(ssql, [], function(err1, rs1) {
        conn.release()
        if (err1) {
          reject(err1)
        } else {
...
var getmany = function (tn, pfilter, trans) {
  return new Promise(function (resolve, reject) {
    var tbldef = module.exports.tbldefs[tn]
    if (tbldef == undefined) {
      console.log('tablename not buffered: ' + tn)
      reject('table name not buffered. so rejected. table name: ' + tn)
      return
    }
    var sselect
    if (trans == undefined) {
      sselect = tbldef["sselect"]
    } else {
      sselect = 'select '
      for (var i in trans) {
    ...
    pool.getConnection(function (err, conn) {
      if (err) {
        ...
        if (tbldef.strtype == strtypeType) {
          console.log('yorm.getmany error for Structure type:' + tbldef.strtype + '. Table name: ' + tn)
          reject('This function only table type supported right now. Typename: ' + tn)
          return
        }
        var sfilter
        var ssql
        try {
          if ((typeof pfilter).toLowerCase() === 'string' && pfilter.trim().length > 0) {
            var words = pfilter.split(' ')
            if (words.length < 2) {
              console.log('Yorm.getmany filter cannot be recornized: ' + pfilter + '. Runtime table name: ' + tn)
              reject('Check your filter: ' + pfilter)
            } else {
              sfilter = pfilter
            }
          } else {
            ...
            for (var key in pfilter) {
              if (pfilter[key] == undefined || pfilter[key] == null) {
                sfilter = sfilter + key + ' is null and'
              } else {
                sfilter = sfilter + key + ' = "' + pfilter[key] + '" and '
              }
              // } else if (isNaN(pfilter[key])){
              //   sfilter = sfilter + key + ' = "' + pfilter[key] + '" and '
              // } else {
              //   sfilter = sfilter + key + ' = ' + pfilter[key] + ' and '
              // }
            ...
            if (sfilter.length > 8) {
              sfilter = sfilter.substr(0, sfilter.length - 4)
            } else {
              sfilter = ' '
            }
          }
          ssql = sselect + tbldef["sfrom"] + " " + sfilter
        } catch (e) {
          console.log('error in yorm.getmany: ' + e)
          reject(e)
        }
        conn.query(ssql, [], function (err1, rs) {
          conn.release()
          if (err1) {
            console.log('Error in sql??: ' + ssql)
            console.log(err1)
            reject(err1);
          } else {
            resolve(rs);
  ...
  return new Promise(function (resolve, reject) {
    ...
    for (var i in pricols) {
      if (colsname.indexOf(pricols[i]) < 0) {
        keymiss = true
        reject(new error('Key missing for table/key: ' + tn + '/' + pricols[i]))
        return
      }
    }
    ...
    // console.log('yorm.saveone ssql:' + ssql)
    pool.getConnection(function (err, conn) {
      // console.log('yorm.saveone Connection got.' + ssql)
      conn.query(ssql, [], function (err1, rs) {
        conn.release()
        if (err1) {
          ...
          // console.log('yorm.saveone query execued: ' + ssql)
          resolve(one)
        }
      })
    })
    ...
    for (var i in arrvalueses) {
      promises.push(function (pI) {
        return new Promise(function (resolve1, reject1) {
          ...
          pool.getConnection(function (err, conn) {
            if (err) {
              console.log('connection error in yorm.savemany(): ' + err)
            } else {
              conn.query(ssql, [], function (err1, rs) {
                conn.release()
                if (err1) {
